...
export function PopoverContent({ className, align = "center", sideOffset = 6, ...props }: PopoverContentProps) {
  return (
    <Primitive.Portal>
      <Primitive.Content align={align} sideOffset={sideOffset} className={cx("z-50 w-72 rounded-control border-2 border-line-strong bg-surface p-4 text-ink shadow-pop outline-none", className)} {...props} />
    </Primitive.Portal>
  );
}
