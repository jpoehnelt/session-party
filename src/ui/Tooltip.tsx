...
export function TooltipContent({ className, sideOffset = 5, ...props }: TooltipContentProps) {
  return (
    <Primitive.Portal>
      <Primitive.Content sideOffset={sideOffset} className={cx("z-50 max-w-xs rounded-control border-2 border-on-ink bg-ink px-3 py-1.5 text-xs font-bold leading-snug text-on-ink shadow-pop", className)} {...props}>
        {props.children}
