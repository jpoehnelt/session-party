...
import { EventOutput } from "../schema";
...
export function EventsWorkspace({ events, now = new Date() }: { events: readonly EventSummary[]; now?: Date }) {
  const orderedEvents = useMemo(() => prioritizeEvents(events, now), [events, now]);
  const featured = orderedEvents[0];
  if (!featured) return null;

  const otherEvents = orderedEvents.slice(1);
      ...
      <section aria-labelledby="featured-event-heading">
        <div className="mb-3 flex items-center justify-between gap-4">
          ...
          <p className="text-[10px] font-black uppercase tracking-[0.12em] text-ink-faint">
            {String(1).padStart(2, "0")} / {String(events.length).padStart(2, "0")}
          </p>
        </div>
        <Card
          ...
          <div className="grid lg:grid-cols-[minmax(0,1.5fr)_minmax(18rem,0.7fr)]">
            <div className="p-6 sm:p-8 lg:p-10">
              ...
              <div className="mt-8 flex flex-wrap gap-3">
                ...
                <Link
                  className="inline-flex min-h-11 items-center border-2 border-line-strong bg-surface px-4 py-2 text-xs font-black uppercase tracking-[0.08em] text-ink shadow-button transition-transform hover:-translate-y-0.5 hover:bg-production-sky focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent focus-visible:ring-offset-3"
                  to={`${featuredBase}/settings`}
                >
                  Event settings
                </Link>
              </div>
        ...
        <section aria-labelledby="other-events-heading">
          ...
          <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-3">
            {otherEvents.map((event, index) => (
              <Link
                className="block h-full text-inherit no-underline transition-transform hover:-translate-y-1 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent focus-visible:ring-offset-4"
                key={event.id}
                to={`/e/${event.slug}`}
              >
