      ...
      <div className={productionTableClass}>
        <Table
          ...
          columns={[
            ...
            { key: "speaker", header: "Speaker / session", render: (asset) => <div><a className="font-bold underline decoration-2 underline-offset-3" href={`/e/${encodeURIComponent(library.event.slug)}/speakers/${encodeURIComponent(asset.speakerId)}`}>{asset.speakerName}</a><p className="text-xs text-ink-muted">{asset.sessionTitles.length > 0 ? asset.sessionTitles.join(", ") : "No session assigned"}</p><p className="text-xs text-ink-muted">{asset.purpose}</p></div> },
