      ...
      <div className={productionTableClass}>
        <Table
          ...
          columns={[
            ...
            { key: "speaker", header: "Speaker / session", render: (asset) => <div><a className="font-bold underline decoration-2 underline-offset-3" href={`/e/${encodeURIComponent(library.event.slug)}/speakers/${encodeURIComponent(asset.speakerId)}`}>{asset.speakerName}</a>{asset.sessionLinks && asset.sessionLinks.length > 0 ? <p className="text-xs text-ink-muted">{asset.sessionLinks.map((session, index) => <span key={session.id}>{index > 0 ? ", " : ""}<a className="underline decoration-1 underline-offset-2 hover:text-accent-deep" href={organizerAgendaTalkPath(library.event.slug, session.id)}>{session.title}</a></span>)}</p> : <p className="text-xs text-ink-muted">{asset.sessionTitles.length > 0 ? asset.sessionTitles.join(", ") : "No session assigned"}</p>}<p className="text-xs text-ink-muted">{asset.purpose}</p></div> },
