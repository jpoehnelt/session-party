...
export class OpenRegistrationStaffUnavailable extends Data.TaggedError("OpenRegistrationStaffUnavailable")<{
  reason?: string;
}> {}
...
export type AppError = NotFound | Unauthenticated | Forbidden | OpenRegistrationStaffUnavailable | Validation | Conflict | External;
...
export const appErrorStatus = (error: AppError): AppErrorStatus => {
  switch (error._tag) {
    ...
    case "Unauthenticated":
      return 401;
    case "Forbidden":
    case "OpenRegistrationStaffUnavailable":
      return 403;
    case "NotFound":
      return 404;
    case "Conflict":
...
export const toPublicAppError = (error: AppError, requestId: string): PublicAppError => {
  switch (error._tag) {
    ...
    case "OpenRegistrationStaffUnavailable":
      return {
        error: error._tag,
        message: "Install-wide staff access is unavailable while open registration is enabled",
        requestId,
      };
    case "NotFound":
